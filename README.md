...
Split sizes and time-series gaps count observations, not elapsed hours. Missing hours are allowed; inputs are never silently sorted or reindexed. Shuffled splitting and fractional time-series test sizes are rejected. These safeguards do not establish disjointness between separately supplied datasets or freeze an untouched final test set.
